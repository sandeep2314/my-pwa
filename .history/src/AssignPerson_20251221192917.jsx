...
export default function AssignPerson() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const resLocations = await fetch(
        `${import.meta.env.VITE_API_URL}/getlocation`,
        { method: "POST" }
      );
      const dataLocations = await resLocations.json();
      setLocations(dataLocations.a || []);

      // Load staff (workers & managers)
      const resStaff = await fetch(
        `${import.meta.env.VITE_API_URL}/getstaff`,
        { method: "POST" }
      );
  ...
  async function submitAssign() {
    ...
    // Hierarchy check (Manager HNo must be >= Worker HNo)
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/addStaffLocation`,
        {
          method: "POST",
